...
class ExperimentConfig:
    ...
    data_path: str = './processed_data/M4T_S.csv'
...
class FeatureManager:
    """Manages feature groups and combinations"""
    
    def __init__(self):
        self.base_features = [f'OR_{i}' for i in range(4)] + \
                            [f'Ac_{i}' for i in range(4)] + \
                            ['SH'] + ['MAPE', 'RD']
...
class ResultsAnalyzer:
    ...
    def generate_feature_combination_tables(self, results_df: pd.DataFrame):
        ...
        import os
        os.makedirs('feature_combo_tables_sequential', exist_ok=True)
        ...
        for combo in feature_combos:
            ...
            combo_latex += f"\\caption{{Performance comparison for feature combination {combo} on sequential data}}\n"
            combo_latex += f"\\label{{tab:combo_{combo.replace('+', '_')}_performance_sequential}}\n\\end{{table}}"
            
            safe_combo_name = combo.replace('+', '_').replace('/', '_')
            with open(f'feature_combo_tables_sequential/{safe_combo_name}_performance.tex', 'w', encoding='utf-8') as f:
                f.write(combo_latex)
            
            logger.info(f"Sequential feature combination table for {combo} saved to 'feature_combo_tables_sequential/{safe_combo_name}_performance.tex'")
    ...
    def generate_feature_combination_summary_table(self, results_df: pd.DataFrame) -> str:
        ...
        summary_latex += "\\\\\n"
        summary_latex += "\\hline\n\\end{tabular}\n"
        summary_latex += "\\caption{Summary of feature combination performance on sequential data}\n"
        summary_latex += "\\label{tab:feature_combo_summary_sequential}\n\\end{table}"
    ...
    def _create_heatmaps(self, summary: pd.DataFrame):
        ...
        for metric in ['ndcg_1', 'ndcg_all']:
            ...
            plt.title(f'{metric_title} Performance of Each Feature Combination (Sequential)', fontsize=14)
    ...
    def _create_comparison_barplot(self, summary: pd.DataFrame):
        ...
        plt.title('NDCG@1 Performance Comparison of Each Feature Combination (Sequential)', fontsize=14)
    ...
    def print_best_combinations(self, summary: pd.DataFrame):
        ...
        print("\n===== Best Feature Combinations (Sequential) =====")
...
class Model4TuneExperiment:
    ...
    def run_experiment(self):
        ...
        logger.info("Starting Model4Tune Sequential experiment...")
        ...
        # Print analysis (now includes LaTeX table generation)
        self._print_analysis(summary, improvement, results_df)
        
        # Create visualizations
        self.analyzer.create_visualizations(summary)
        
        logger.info("Sequential experiment completed successfully!")
    ...
    def _save_results(self, results_df: pd.DataFrame, summary: pd.DataFrame, improvement: pd.DataFrame):
        ...
        results_df.to_csv('feature_combination_results_sequential.csv', index=False)
        summary.to_csv('feature_combination_summary_sequential.csv')
        improvement.to_csv('feature_combination_improvement_sequential.csv')
    ...
    def _print_analysis(self, summary: pd.DataFrame, improvement: pd.DataFrame, results_df: pd.DataFrame):
        ...
        print("\n===== Performance comparison of each feature combination (Sequential) =====")
        print(summary)
        
        print("\n===== Performance improvement percentage relative to random (Sequential) =====")
        print(improvement[improvement.index != 'random'].round(2))
        
        self.analyzer.print_best_combinations(summary)
        
        print("\n===== Generating feature combination summary LaTeX table (Sequential) =====")
        summary_latex = self.analyzer.generate_feature_combination_summary_table(results_df)
        print(summary_latex)
        
        print("\n===== Generating individual feature combination tables (Sequential) =====")
